...
const RightSidebar: React.FC<RightSidebarProps> = ({ playlists, setSelectedPlaylist, selectedPlaylist }) => {
  return (
    <div className="w-3/4 bg-gray-900 p-4">
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {playlists.map((playlist, index) => (
          <div
            key={index}
            className={`cursor-pointer p-4 rounded-lg shadow-md ${selectedPlaylist?.title === playlist.title ? 'bg-blue-700' : 'bg-gray-800'}`}
